...
const PosLeftCard = ({
  ...
  const canCheckout =
    Number(studentsData?.deposite_amount || 0) >= total && total > 0;
        ...
        {findSingleStudent && studentsData ? (
          ...
          >
            <Typography variant="body2">
              Student Name: <strong>{studentsData?.student_name} {studentsData?.father_name}</strong> ({studentsData?.class_info?.class_name + " - " + studentsData?.class_info?.section + " " + studentsData?.class_info?.academic_year})
            </Typography>

            <Typography variant="body2">
              Balance:{" "}
              <span className="text-green-500 font-semibold">
                ₹{studentsData?.deposite_amount}
              </span>
            </Typography>
          </Box>
        ) : null}
